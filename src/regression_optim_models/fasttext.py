...
# Average CV score on the training set was: -644.458711443802
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    StackingEstimator(estimator=RidgeCV()),
    ExtraTreesRegressor(bootstrap=False, max_features=0.6500000000000001, min_samples_leaf=2, min_samples_split=18, n_estimators=100)
)
...
with open("{}summary_performance_fasttext.json".format(path_export), 'w') as doc_export:
    json.dump(dict_values, doc_export)

name_export = "{}fasttext_model.joblib".format(path_export)
