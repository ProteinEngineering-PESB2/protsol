...
# Average CV score on the training set was: -696.8681994243036
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    ZeroCount(),
    StackingEstimator(estimator=SGDRegressor(alpha=0.001, eta0=0.1, fit_intercept=True, l1_ratio=0.0, learning_rate="invscaling", loss="epsilon_insensitive", penalty="elasticnet", power_t=50.0)),
    ExtraTreesRegressor(bootstrap=False, max_features=0.2, min_samples_leaf=1, min_samples_split=19, n_estimators=100)
)
...
with open("{}summary_performance_glove.json".format(path_export), 'w') as doc_export:
    json.dump(dict_values, doc_export)

name_export = "{}glove_model.joblib".format(path_export)
