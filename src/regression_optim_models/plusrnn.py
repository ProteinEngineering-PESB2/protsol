...
# Average CV score on the training set was: -651.5474634599318
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    StackingEstimator(estimator=RidgeCV()),
    Normalizer(norm="l1"),
    SelectFwe(score_func=f_regression, alpha=0.004),
    ExtraTreesRegressor(bootstrap=False, max_features=0.15000000000000002, min_samples_leaf=7, min_samples_split=4, n_estimators=100)
)
...
with open("{}summary_performance_plusrnn.json".format(path_export), 'w') as doc_export:
    json.dump(dict_values, doc_export)

name_export = "{}plusrnn_model.joblib".format(path_export)
